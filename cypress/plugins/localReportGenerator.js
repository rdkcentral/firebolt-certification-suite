...
async function generateLocalReport(reportObj, jobId) {
  ...
  if (reportObj.mochawesomeReport) {
    ...
    logger.info(
      `A local report has been generated and can be accessed at ./reports/${jobId}/mochawesome/mochawesome-report.html`,`generateLocalReport`
    );
  }
  ...
  if (reportObj.cucumberReport && reportObj.cucumberReportFilePath) {
    ...
    logger.info(
      `A local report has been generated and can be accessed at ./reports/${jobId}/cucumber-html-report/index.html`,`generateLocalReport`
    );
  }
}
...
function removeTagsFromCukeHtml(htmlReportPath) {
  ...
  if (table) {
    ...
    logger.info(`Table with id "features-table" not found in the HTML.`,`removeTagsFromCukeHtml`);
  }
}
