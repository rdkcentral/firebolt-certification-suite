...
export default class FireboltInvoker {
  ...
  buildWebSocketUrl(deviceHost, wsUrlProtocol, port, path) {
    ...
    return `${protocol}${deviceHost}:${wsPort}/${wsPath}`;
  }

  async get() {
    if (this.instance === null) {
      const wsUrl = this.buildWebSocketUrl(Cypress.env("deviceHost"), Cypress.env("wsUrlProtocol"), Cypress.env("wsPort"), Cypress.env("wsUrlPath"));
  ...
  async invoke(methodName, params) {
    ...
    try {
      let currentInstance = await this.get();
      let response = await currentInstance.send(JSON.stringify(fireboltMessage));
