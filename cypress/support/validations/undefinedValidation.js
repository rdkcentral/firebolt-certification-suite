...
Cypress.Commands.add('undefinedValidation', (validationTypeObject, apiOrEventObject, validationType) => {
    ...
    try {
      ...
      validationTypeObject.validations.forEach((validation) => {
        ...
        if (validation.field === CONSTANTS.RESULT) {
          assert.isUndefined(methodOrEventResponse.result, validation.description);
        } else {
          ...
          for (let i = 0; i < fieldParts.length - 1; i++) {
            ...
            if (!currentObject || typeof currentObject[part] === CONSTANTS.UNDEFINED) {
              logger.error(`Expected ${methodOrEventName} response to have property ${part} in the field "${validation.field}`, 'undefinedValidation');
            }
            currentObject = currentObject[part];
          }
          // Else assert if provided field is undefined
          const finalPart = fieldParts[fieldParts.length - 1];
          firelog.isUndefined(currentObject[finalPart], `Expected ${methodOrEventName} response to have the field "${validation.field} as undefined`);
        }
      });
    } catch (error) {
      cy.log(
