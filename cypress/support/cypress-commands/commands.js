...
Cypress.Commands.add('methodOrEventResponseValidation', (validationType, requestData) => {
  const { method, context, contentObject, expectingError, appId, eventExpected, isNullCase } = requestData;
      ...
      cy.then(() => {
        if (validationType == CONSTANTS.EVENT) {
          ...
          eventResponse.then((response) => {
            ...
            cy.updateResponseForFCS(method, null, response, true, isNullCase).then((updatedResponse) => {
