...
Feature: Lifecycle_Foreground
    ...
    @Lifecycle @coreSDK
    Scenario: Lifecycle R*3.2 Loading & Launching an App (initializing -> foreground)
    ...
    @Lifecycle @coreSDK
    Scenario: Lifecycle R*3.2.3 Relaunch foreground app
        Given the environment has been set up for 'lifecycle' tests
    ...
    Scenario: Lifecycle R.3.2.1 App Launching another app
        Given the environment has been set up for 'lifecycle' tests
        And 3rd party 'certification' app is launched with 'foreground' state
