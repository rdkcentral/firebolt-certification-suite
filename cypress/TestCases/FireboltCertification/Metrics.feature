...
Feature: Metrics
    ...
    Scenario Outline: Metrics.<Method> - Validating API Error handling when <Scenario>
        ...
        Examples:
            ...
            | page has a emptyParam                 | page                 | notify that page has navigated with empty parameter            | invalid params for metrics page                 |
            | action has a emptyParam               | action               | notify about action with empty parameter                       | invalid params for metrics action               |
            ...
            | mediaLoadStart has a emptyParam       | mediaLoadStart       | infer load time with empty parameter                           | invalid params for metrics mediaLoadStart       |
            | mediaPlay has a emptyParam            | mediaPlay            | start playback with empty parameter                            | invalid params for metrics mediaPlay            |
            | mediaPlaying has a emptyParam         | mediaPlaying         | notify that playback has started with empty parameter          | invalid params for metrics mediaPlaying         |
            | mediaPause has a emptyParam           | mediaPause           | notify that playback has paused with empty parameter           | invalid params for metrics mediaPause           |
            | mediaWaiting has a emptyParam         | mediaWaiting         | notify that playback has halted with empty parameter           | invalid params for metrics mediaWaiting         |
            | mediaProgress has a emptyParam        | mediaProgress        | notify that playback is progressing with empty parameter       | invalid params for metrics mediaProgress        |
            | mediaSeeking has a emptyParam         | mediaSeeking         | notify that playback seek is initiated with empty parameter    | invalid params for metrics mediaSeeking         |
            | mediaSeeked has a emptyParam          | mediaSeeked          | notify that playback seek is completed with empty parameter    | invalid params for metrics mediaSeeked          |
            | mediaRateChange has a emptyParam      | mediaRateChange      | notify that playback rate is changed with empty parameter      | invalid params for metrics mediaRateChange      |
            | mediaRenditionChange has a emptyParam | mediaRenditionChange | notify that playback rendition is Changed with empty parameter | invalid params for metrics mediaRenditionChange |
